...
export default function ExpenseCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setCategories(await getFinanceCategories('expense', storeId));
    } catch {
      setCategories([]);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingCat) {
        await updateFinanceCategory('expense', editingCat.id, {
          name: formName.trim(),
        ...
        await createFinanceCategory('expense', {
          storeId,
          name: formName.trim(),
          description: formDescription.trim() || undefined,
          color: formColor || undefined,
  ...
  const handleDelete = async (cat: FinanceCategory) => {
    ...
    setMessage(null);
    try {
      await deleteFinanceCategory('expense', cat.id);
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-4 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-sm text-neutral-400">
            Clasifica tus <span className="font-medium text-red-400">compras</span> y <span className="font-medium text-red-400">cuentas por pagar</span> por categoría
          </p>
        </div>
      ...
      <div className="mb-4 rounded-xl border border-red-500/20 bg-red-500/5 p-4 sm:mb-6">
        <p className="text-sm text-red-300">
          Estas categorías se usan para clasificar <strong>compras</strong> y <strong>cuentas por pagar</strong> (egresos de tu negocio).
          No se aplican a productos ni a ventas.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/80 p-8 text-center backdrop-blur-sm sm:rounded-3xl sm:p-12">
          ...
          <h3 className="mb-2 text-lg font-medium text-neutral-200 sm:text-xl sm:font-light">
            Sin categorías de egresos
          </h3>
          <p className="mb-6 text-sm text-neutral-400 sm:text-base">
            Crea categorías para organizar tus compras y cuentas por pagar.
          </p>
          ...
          >
            ...
            >
              <h2 className="mb-4 text-lg font-medium text-neutral-100">
                {editingCat ? 'Editar categoría de egresos' : 'Nueva categoría de egresos'}
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="mb-1 block text-xs text-neutral-500">Nombre *</label>
                  <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Ej: Alquiler, Servicios, Inventario..." className={inputClass} />
                </div>
                <div>
